...
public class Capture implements Runnable{
	...
	protected XBimuSample mainProgram;
	protected long currentNanos;
	protected SerialPort serialPort = null;
	protected BufferedOutputStream oStream = null;
	DrawImage drawImage1 = null;
	DrawImage drawImage2 = null;
	Boolean keepSampling = true;
	/*Constructor*/
	public Capture(XBimuSample mainProgram,SerialPort serialPort,BufferedOutputStream oStream, DrawImage drawImage1,DrawImage drawImage2){
	...
	public void run() {
		try{
			...
			while (keepSampling) {
				serialPort.writeByte((byte)1);	/*Write 1 to let the arduino know that we're good to go*/
				//System.out.println("Start sampling");
				while (keepSampling && serialPort.getInputBufferBytesCount() <bufferIn.length){
					try{
				...
				if (serialPort.getInputBufferBytesCount() >= bufferIn.length){
					bufferIn = serialPort.readBytes(bufferIn.length);
				}else{
				...
				if (bufferIn != null){
					...
					while (checkSum != checkSumReceived) {
						...
						tempByteBuffer = serialPort.readBytes(1); //Read one more byte of data
					...
					if (oStream != null && mainProgram.continueSampling){
						...
						try{
							...
							oStream.write(tStampBytes);	/*Write straight from the buffer*/
							...
							oStream.write(zeroBytes);	/*Write straight from the buffer*/
							oStream.flush();
						} catch (Exception err){
					...
					if (plotCount >= plotEveryNth){
						...
						/*Plot the traces*/
						for (int i = 0; i < noChannels;++i){
							drawImage1.plotTrace(data.get(i).toArray(new Integer[]{}),i,zeros[i]);
						}
						drawImage1.paintImageToDraw();
						drawImage2.plotNumber((float) score);
	...
	public void setOStream(BufferedOutputStream oStreamIn){
		try{
	...
	protected void doneSampling(){
		...
		try{
			serialPort.writeByte((byte)2);	/*Write 2 to let the arduino know that we're done*/
		}catch (SerialPortException ex){
			System.out.println(ex);
		}
		/*Close the save file*/
